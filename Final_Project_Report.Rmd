...
\newpage
## Dataset
Our dataset is a compilation of houses in Ames Iowa. It has 79 explanatory variables that may be helpful in determining the sale price on a home. Our group is looking into creating a model using the significant variables in the dataset to determine house price using the features of the home. Some important variables are SalePrice: the property's sale price in dollars, which is the variable we are trying to predict, LotArea: Lot size in square feet, OverallCond: Overall condition rating, and YearBuilt: Original construction date. Our main goal is to gain insights on what the most significant predictors of SalePrice are and to build a predictive model.
